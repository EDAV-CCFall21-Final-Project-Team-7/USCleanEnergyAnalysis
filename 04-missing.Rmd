...
The plot below shows the time series of different types of solar energy production. From the plot it can be noticed that the "all_solar" fuel type, which is the aggregation of the "all_utility_scale_solar" and "small_scale_solar_photovoltaic" fuels, is missing data for dates before 2014. It seems that the aggregation did not include the all_utility_scale_solar values before that time; perhaps the aggregation had an error because there were n/a's in the data for small_scale_solar_photovoltaic during this time period.

```{r, echo = F}
# df <- read.csv('data/raw/clean_monthly_fuel_generation_.csv')
# df$fuel <- trimws(df$fuel) 
# df$date <- as.Date(df$date)
# df[is.na(df$thousand_megawatthours),]$thousand_megawatthours <- 0
# df$fuel <- gsub(" ", "_", df$fuel)
...
df  %>% 
  group_by(fuel, date) %>% summarise(thousand_megawatthours = sum(thousand_megawatthours)) %>% filter(fuel %in% c('all_solar', 'all_utility_scale_solar', 'small_scale_solar_photovoltaic')) %>%
...
#  annotate("text", x = as.Date('2013-01-01'), y = 2500, label = "all_solar aggregated incorrectly before this #point.")
```

## The World Bank (WB) Data
### Missing values analysis of the WB data

We examined missing data in the WB data set  from two perspectives - by data fields (column) and years (row). The tables of top 10 missing values by rows/columns, heat map of missing values analysis by data fields , and find missing values pattern by year plot below:
