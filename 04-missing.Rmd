...
The EIA.gov data set was the only data set with missing values that we used in our analysis. Most of the missing values were absent because most states did not generate electricity for all fuel types. This missing data can show which fuel types are most widely adopted by all U.S. states and which fuel types are less common. 
...
The plot below shows the time series of different types of energy production. From the plot it can be noticed that not all energy generated starts from the same dates. We can notice that the solar data starts only around 2014. Therefore, we can say that the solar energy data is missing for all dates before that. 
