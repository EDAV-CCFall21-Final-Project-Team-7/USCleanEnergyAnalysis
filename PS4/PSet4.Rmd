...
annotation <- data.frame(x = 3, y = , label = 'complete cases')
...
find_missing_patterns <- function(df, aggregation = 'count'){
  ...
  ggarrange(g1,blank,g2,g3,ncol=2,nrow=2,widths = c(8,4),heights=c(1,4))
  return(missing_patterns_count)
}
```

```{r,fig.width=12,fig.height=12}
test <- find_missing_patterns(mycars, 'count')
```
...
1. Create GitHub - Raihan created an organization and set up the final project repository following the EDAVproject template and added team members to the repo (gnyirjesy, Abhimanyu210100, raihan-bashir)

2. Acquire data/setup API connections
      a. Gabby set up API key for EIA.gov
      b. Abhimanyu downloaded Yahoo Finance data files
      c. Raihan downloaded World Bank data file

3. Data analysis for the respective datasources and collaborate on missing data analysis
